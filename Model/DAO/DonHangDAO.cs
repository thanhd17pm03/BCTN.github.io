...
namespace Model.DAO
{
    public class DonHangDAO
    {
        ...
        {
            IQueryable<DONHANG> model = db.DONHANGs.Where(x => x.TinhTrang == 0);
        ...
        {
            IQueryable<DONHANG> model = db.DONHANGs.Where(x => x.TinhTrang == 1);
            List<OrderViewModelForIndex> listodvm = new List<OrderViewModelForIndex>();
            if (!string.IsNullOrEmpty(searchString))
            {
                model = model.Where(x => x.TenKhachHang.Contains(searchString) || x.DiaChiKhachHang.Contains(searchString));
        ...
        {
            IQueryable<DONHANG> model = db.DONHANGs.Where(x => x.TinhTrang == 2);
            List<OrderViewModelForIndex> listodvm = new List<OrderViewModelForIndex>();
            if (!string.IsNullOrEmpty(searchString))
            {
                model = model.Where(x => x.TenKhachHang.Contains(searchString) || x.DiaChiKhachHang.Contains(searchString));
